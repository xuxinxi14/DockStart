...
本文档说明 DockStart Python runtime 的设计和边界。Basic profile 随附精简后端 runtime；Assisted profile 额外随附固定、独立、可替换的 RDKit/Meeko 工具链，并保留用户配置优先级。profile 名称与版本成熟度分开记录。
...
DockStart 现在区分两类 Python：

1. 桌面端调用后端模块时使用的 **backend runtime Python**；
2. RDKit/Meeko 自动准备 PDBQT 时使用的 **preparation toolchain Python**。

backend runtime Python 的解析优先级为：

```text
bundled > configured > current_environment
```

这样安装包可以优先使用随包的轻量 Python runtime 运行 DockStart 后端，减少用户首次启动时的系统 Python 依赖。

preparation toolchain Python 的解析优先级为：
...
Basic 分发包的轻量 Python 只运行 DockStart 后端，不含 RDKit/Meeko。Assisted profile 已提供离线 fallback；独立 conda 环境只在用户需要替换版本或自管工具链时推荐。
...
V0.6.2 新增 `scripts/export_toolchain_environment.py`，用于读取 DockStart configured Python 并导出轻量 conda 环境描述。如果 configured Python 不属于 conda 环境，脚本会返回结构化中文提示，不会安装包、修改 PATH 或复制环境目录。推荐文档见 `docs/release/toolchain_environment.md`。

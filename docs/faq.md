...
不能。V0.3.0 只建立 `project.json` 中的 `preparation` 数据模型、准备状态读取、前置检查和状态重置入口。V0.3.1 进一步检测 RDKit/Meeko 的 import、版本和准备能力是否可确认，但仍不会自动生成 ligand/receptor PDBQT。真实 PDBQT 自动生成会在后续阶段单独实现，并且仍需要用户检查科学合理性。

...
DockStart v0.14.0 的目标是交付 Basic Stable、Assisted Stable，以及保持协议隔离的高级与实验性工作流。独立 AutoDock4
maps 工作流继续可用，不修改 docking 算法或扩大科学结论能力。

## Git 与版本

- 当前分支为 `main`，`git status --short` 干净；
- 以下七处版本均为 `0.14.0`：后端 `__init__.py`、`package.json`、
...
- MSI/NSIS 文件名只包含当前版本，大小和 SHA256 记录到本轮 `v0.14.0` 构建报告；

...
DockStart v1.0.3 是非最终的本地候选版本。Basic 与 Assisted 是两个隔离的发布 profile，
...
- 后端 `__init__.py`、`package.json`、`package-lock.json`、`Cargo.toml`、`Cargo.lock`、
  `tauri.conf.json`、`pages.ts` 七处权威版本必须全部为 `1.0.3`；
...
- v1.0.3 的所有构建结果均保持 `candidate=true`、`publishable=false`。通过候选门禁只允许写为
...
v1.0.3 打包必须运行 `scripts/check_all.ps1` 定义的完整自动检查；完整回归仍不能替代独立外部科学门禁：
...
- `-SkipPostInstallGate` 产物只能是 `candidate_incomplete`；即使 post-install 通过，v1.0.3 仍为
...
- 明确 Basic/Assisted 是 profile，v1.0.3 的成熟度是非最终本地候选；

...
DockStart v0.14.2 是非最终的本地候选版本。Basic 与 Assisted 是两个隔离的发布 profile，
...
- 后端 `__init__.py`、`package.json`、`package-lock.json`、`Cargo.toml`、`Cargo.lock`、
  `tauri.conf.json`、`pages.ts` 七处权威版本必须全部为 `0.14.2`；
...
- v0.14.2 的所有构建结果均保持 `candidate=true`、`publishable=false`。通过候选门禁只允许写为
...
v0.14.2 打包必须运行构建脚本定义的完整自动检查；完整回归仍不能替代独立外部科学门禁：

```powershell
python -m py_compile scripts/ad4_v0141_common.py `
  scripts/verify_ad4_flexible_1fpu.py `
  scripts/verify_ad4_multiple_ligands_5x72.py `
  scripts/verify_ad4_serial_screening.py `
  scripts/verify_ad4_v0141_evidence_bundle.py

python -m unittest `
  backend.tests.test_ad4_v0141_evidence_bundle `
  backend.tests.test_scientific_fixtures

python -m unittest `
  backend.tests.test_multiple_ligands.MultipleLigandParserTests.test_log_output_score_comparison_allows_only_display_rounding `
  backend.tests.test_screening.ScreeningWorkflowTests.test_standard_ad4_batch_freezes_maps_runs_and_archives

Push-Location apps/desktop
npm run test:frontend:async -- tooltipAccessibility.test.ts markdownPreview.test.ts structureInput.test.ts
npx tsc --noEmit
Pop-Location
```
...
- `-SkipPostInstallGate` 产物只能是 `candidate_incomplete`；即使 post-install 通过，v0.14.2 仍为
...
- 明确 Basic/Assisted 是 profile，v0.14.2 的成熟度是非最终本地候选；

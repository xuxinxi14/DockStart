...
- 受体：通过 RCSB PDB ID 获取 PDB/CIF；
- 配体：通过 PubChem CID 或名称获取 SDF。
...
PDB/SDF 到 PDBQT 并不只是修改扩展名。DockStart 会调用 RDKit/Meeko 完成最小准备，但结果仍需要人工科学检查。
...
| 受体原始结构 | PDB、CIF |
| 配体原始结构 | SDF、MOL |
| 在线结构来源 | RCSB PDB ID、PubChem CID/名称 |

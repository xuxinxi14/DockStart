...
既有统一验收已完成后端基线 `1191/1191`、前端 `86/86`、Rust `32/32` 和 TypeScript/Vite 生产构建；随后发现并补齐的证据输出预约增量又通过 BACE/评价 verifier 专项 `20/20`。评价模式、Vina/Vinardo maps、100 项批筛与 BACE_1 的独立真实 verifier 均有对应证据。v0.14.0 新增的三条标准 AD4 路径先执行针对性后端回归与生产构建，并生成本地 GUI 候选供真实人工体验；本轮不重复完整发布门禁，也不把候选包称为公开 Release。安装生命周期与首个公开 Release 继续保持延期。
...
### 三级补充：v0.14.0 标准 AD4 扩展

状态：**三条源码闭环已接入，按 v0.14.0 本地 GUI 候选进行人工验收**。
...
- AutoGrid4 仍是用户配置的 GPL 外部工具；v0.14.0 安装包只包含调用、校验、冻结和结果工作流，不分发 AutoGrid4。

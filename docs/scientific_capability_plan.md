...
## 当前科学功能收口状态（2026-08-05 审计）

本节是当前完成路径的权威摘要。下文按能力形成过程保留的旧版本号、打包状态和“待办”文字仅作历史记录；如与本节冲突，以本节和当前代码、真实工具链证据为准。当前顺序固定为：先完成科学功能与证据，再统一测试，然后才进入实际体验、UI 优化、打包与首个公开 Release。

### 当前科学功能已经补齐

1. **水合 AD4 正向姿势恢复**：固定 1UW6 共晶参考、显式原子映射和同一受体坐标系的无拟合重原子 RMSD 门禁已经接入。当前项目 API 真机运行的 Top 3 最佳为 Mode 2、`0.356053 Å`，通过预先冻结的 `≤ 2.0 Å` 门槛。2BYS/2ZJU 的 `12.760 Å` / `6.422 Å` 继续作为负面姿势诊断，不改写成科学成功，也不引入未定义的 dry/processed affinity。
2. **评价模式与高级参数外部验收**：固定 1IEP 输入已真实执行全局对接、`score_only` 和双阶段 `local_only`；运行结果、能量分解、适用参数、命令、PID 身份、执行期 Vina 起止哈希及不可变二进制证据共同受门禁。2026-08-01 的真实验收 JSON SHA256 为 `9a1cb9ada67c34333a2348209df071ad2d68322024e26103ade78b7cc08860a7`。
3. **Vina/Vinardo 预计算 maps 外部验收**：两个评分协议均已覆盖真实 `--write_maps`、激活、冻结、`--maps` 对接、结果/报告、真实进程失败与重试、同长度 map 篡改与恢复；固定输入、自定义 probe、工具与输入 TOCTOU、命令/PID/进程证据和独立结果 oracle 已分开处理。2026-08-01 的真实验收 JSON SHA256 为 `62d366e04e341b07714de94c7c095eb3260a723600cc0a49177823c92195e6c3`。
4. **BACE_1 大环完整链**：真实 Meeko 审查准备、20-Mode Vina 对接和 `mk_export` 已完成；独立 RDKit 复读确认所有 20 个 SDF pose 均无 `G*`，保持 38 个重原子、39 条重原子键、总形式电荷 0 和固定 canonical isomeric SMILES。固定 Vina 1.2.7 二进制、执行命令/PID/进程身份、执行期起止哈希和 `snapshots.inputs` 受体/配体证据门禁已真实重放通过；2026-08-01 的证据 JSON SHA256 为 `d887235ad81514fea509d8301000a953efa4809af8b93448e3685adf8078374e`。
5. **100 项串行批筛规模证据**：固定工作负载已启动 100 个真实 Vina 进程，在第 20 项运行期间请求取消并恢复同一队列；最终 100 项全部成功且确定性结果一致。实测墙钟 `125.931 s`、峰值进程树内存 `372,428,800 bytes`、取消请求延迟 `47.128 ms`，证据 JSON SHA256 为 `528737a057f8459b0ee4c7dc6c066a608061dc939d8d7bbd3a8249744fceecb`。该结果支持当前串行模型，不构成立即开发并行执行的依据；500 项仅作为显式确认后的容量 probe，不是当前科学功能缺口。
6. **v0.14.0 标准 AD4 扩展**：有限柔性单配体以刚性分量生成 maps，并在运行时加载匹配的柔性侧链；串行批量以一组冻结 maps 逐配体运行；双配体共同对接以两个有序配体执行一次联合搜索。三条路径都校验 map manifest、文件 SHA256 和配体原子类型覆盖，且不改变 Vina/AD4 评分算法。
7. **v0.14.1 外部验收收口**：1FPU 有限柔性 AD4、5X72 双配体联合 AD4 与固定 12 项串行 AD4 三条独立、离线、失败即阻断的验收器已用固定官方输入、Vina 1.2.7、Meeko 0.7.1 与调用者固定 SHA256 的 AutoGrid 4.2.6 实跑通过。通过 JSON SHA256 依次为 `9dd3a3e810c365b9d68e3575e51379fa4e7b3ce8a101a434dada9e0a11ce739a`、`4cbc16810e4a65b154b684be5c2bee60b2ca54526a978ceb5f24aa876726b9f0`、`af8baeb84ae34b414bc643897c0bd8d72558dc42af9023b59e1ad4290c0a8d75`。

### P1：当前科学功能与统一验收已收口

既有统一验收已完成后端基线 `1191/1191`、前端 `86/86`、Rust `32/32` 和 TypeScript/Vite 生产构建；随后发现并补齐的证据输出预约增量又通过 BACE/评价 verifier 专项 `20/20`。评价模式、Vina/Vinardo maps、100 项批筛、BACE_1 与三条标准 AD4 独立真实 verifier 均有对应证据。v0.14.1 完成稳定化和三条必要外部门禁；v0.14.2 只重新生成 Basic 与 Assisted 本地候选，不改变科学协议。完整安装生命周期与首个公开 Release 继续保持延期。
...
4. **水合 AD4 工程链**：1UW6 正向门禁、2BYS/2ZJU 真机工程验收、大网格、并发、终止/恢复、篡改和结果序列化门禁已经通过；协议仍保持 Experimental，以便继续积累不同水环境、Box/spacing 和工具版本的成熟度证据。
...
- 自定义评分权重；
- Python API / 协议插件；
- 批量并行执行（100 项资源基准不显示必须并行；仅在新的吞吐需求和 500 项容量证据支持时立项）；
...
- 标准 AD4 有限柔性单配体、刚性串行批量与刚性双配体共同对接；
...
- 正式 Meeko 大环 schema v2：候选断环、人工确认、刚性选择、显式氢/原子映射、键拓扑、工具版本、不可变合同、fail-closed 发布、run 快照、报告和 3D 候选定位；已用 Assisted RDKit 2026.03.3、Meeko 0.7.1、Vina 1.2.7 对 BACE_1 完成 20-Mode 真实项目链、执行期工具/输入门禁与逐 pose 拓扑复读。该项完成不代表已经打包或发布。
...
- **真实工具链回归（持续补强）**：验收证据已覆盖批筛失败隔离/取消恢复/100 项资源负载、1FPU 柔性对接、BACE_1 大环和标准 AutoGrid4/AD4；这些证据不等于干净账户安装、系统重启和卸载生命周期验收。
...
2. 已完成 100 项真实 Vina 串行资源基准、运行中取消请求和同队列恢复；实测峰值进程树内存约 355 MiB，当前没有证据要求立即引入并行执行。500 项保留为用户明确需要更大容量时的可选 probe，不能在现有上限 500 的情况下写“1000 项已支持”。
...
1. Assisted Python 3.11.15、RDKit 2026.03.3、Meeko 0.7.1 与 Vina 1.2.7 已对 BACE_1 完成 schema v2 真实项目链：生成 7 个候选，确认 `candidate_72113262ddb85177`，预期/实际断开 C3—C4，G* 为 2；固定高强度运行得到 20 个 Mode，`mk_export` 后由独立 RDKit 逐 pose 确认无 G* 且重原子图、形式电荷和 canonical isomeric SMILES 保持一致。执行期 Vina 身份、二进制起止哈希与冻结受体/配体快照也已在同一次最终 verifier 中通过。
...
历史 v0.12.0 打包基线的标准 AD4 工作流边界只开放非金属、刚性受体、单配体协议。后续源码中的 Zn 专用能力进入四级 AD4Zn beta；水合 AD4 已形成独立 Experimental 闭环，但不属于旧安装包。v0.14.0 又开放标准 AD4 的有限柔性单配体、刚性串行批量与刚性双配体共同对接；AD4Zn、水合 AD4 与这些路径的组合仍不开放。

### 三级补充：v0.14.0 标准 AD4 扩展 / v0.14.1 验收收口

状态：**三条源码闭环与独立外部验收均已在 v0.14.1 非最终候选上实跑通过；本轮未重新打包**。

- 有限柔性 AD4 的 map set 绑定当前柔性准备记录、刚性分量、柔性侧链、残基选择和 SHA256；运行时使用冻结的 `--maps` 与 `--flex`，不是把完整柔性受体直接交给 AutoGrid4。
- 串行批量 AD4 在建队时冻结一份 map set，并在运行前验证其覆盖整个配体库的 PDBQT 原子类型；每个 attempt 仍有独立配置、命令、输入哈希、输出和恢复状态。
- 双配体共同 AD4 只接受恰好两个有序配体，使用官方 `--ligand <A> <B>` 语义和同一组 `--maps ... --scoring ad4`；报告保存联合评分，不生成虚假的成员分值。
- 批量与共同对接只允许刚性受体、全局任务和标准 AD4；AD4Zn beta、水合 AD4 Experimental、评价模式及 Vina/Vinardo 预计算 maps 不会被静默转换成标准 AD4。
- AutoGrid4 仍是用户配置的 GPL 外部工具；v0.14.1 本轮没有生成安装包，任何后续 Basic/Assisted 候选也不得分发 AutoGrid4。

### 三级补充：Vina / Vinardo 预计算 maps 复用（当前源码增量）

状态：**源码闭环已接入 v0.14.0 本地候选包，尚未完成公开发布门禁**。
...
状态：**AD4Zn beta 已接入 v0.14.0 本地候选包，尚未完成公开发布门禁**。
...
1. 1UW6 已以预先冻结的参考构象、显式原子映射和 `≤ 2.0 Å` 阈值取得正向姿势恢复，Top 3 最佳 Mode 2 为 `0.356053 Å`。2BYS/2ZJU 继续保留为负面姿势诊断，不冒充正向证据。
2. 扩大不同配体、口袋含水环境、Box/spacing 和 Vina/Meeko/AutoGrid 版本组合属于从 Experimental 向 Stable 演进的成熟度积累，不再作为当前协议主链缺口。1UW6 加两套负面姿势诊断仍不足以支持 Stable 声明。
3. 大网格、并发生成、终止/恢复、maps 替换、迁移和模拟写入失败已经覆盖；仍可在后续硬件基准中补充物理磁盘不足与资源上限测量，但不再把这些写成协议主链未实现。

Basic/Assisted 干净安装、升级、重启、卸载、长中文/空格路径、亮暗主题截图、用户指南和发布材料属于科学闭环后的体验与发布阶段，本阶段不执行。水合协议即使已经通过最低正向科学基准，也继续保持 Experimental，不进入默认入门流程，不能与 Stable 协议结果混排；升级成熟度需要更广的独立体系和工具版本证据。

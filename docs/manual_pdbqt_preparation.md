...
Meeko/RDKit 当前不是 DockStart 仓库内置的完整 runtime。V0.3 自动准备只会使用已经检测到的 Python + RDKit/Meeko 环境；DockStart 不会自动安装这些包，也不会提交 `python.exe`、`Lib/`、`DLLs/` 或 `site-packages/`。如需把 RDKit/Meeko 真正纳入 Full 版内置工具链，仍需要单独审查许可证文本、依赖来源、包体积、更新机制和测试覆盖。

...
V0.3.2 可以在检测到可用 Python + RDKit + Meeko 时，尝试把 ligand SDF/MOL raw 文件准备为 `prepared/ligand.pdbqt`。DockStart 当前仍不会自动把 receptor PDB/CIF、ligand MOL2 或 SMILES 转成 PDBQT，也不会接入 Open Babel、PLIP 或 MGLTools。
...
这些步骤涉及化学和结构判断。DockStart V0.3.2 只对 ligand SDF/MOL 提供最小自动准备辅助；受体准备、MOL2/SMILES 和复杂结构修复仍需用户使用外部工具或后续版本。DockStart 不自动判断准备结果是否科学合理。
...
## 8. DockStart 当前不保证外部 PDBQT 的科学正确性

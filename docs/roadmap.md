...
V0.4.1 已接入最小前端 3Dmol.js ViewerPage，使用 npm 本地依赖，不使用 CDN。V0.4.2 已完成 Box overlay 数据与 `project.json.box` 保存同步。V0.4.3 已完成 docking pose mode 查看和 `scores.csv` 摘要对应。workflow status 的 viewer 推荐入口仍在 V0.4.4 之后继续推进。

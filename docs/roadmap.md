...
- ToolchainStatusPage 明确 Meeko/RDKit 当前只做 import 检测，不会自动处理分子。
...
- RDKit/Meeko 自动准备方案设计；
- raw receptor / ligand 到 prepared PDBQT 的数据模型设计；
- 输入校验、错误提示和可追溯记录设计；
- 许可证、体积、离线包和更新机制审查；
- mock-first 测试方案；
...
- V0.3.2 之后，ligand SDF/MOL 可以尝试自动准备为 PDBQT；receptor 自动准备、MOL2/SMILES 和复杂结构修复仍需后续阶段实现；

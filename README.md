...
DockStart 的长期目标是成为中文友好、流程清晰、可复现的分子对接工作台。当前 V0.9.7 发布目标是 **Basic Stable**：随应用提供 AutoDock Vina 与 DockStart 后端 Python，用户导入已准备好的 `receptor.pdbqt` 和 `ligand.pdbqt` 后，即可离线完成 Box 设置、Vina 对接、结果解析和报告导出。
...
- 当前发布目标为 V0.9.7 Basic Stable。该版本不随包提供 RDKit/Meeko；Assisted Mode 的界面和调用接口仍然保留，需要用户配置带 RDKit/Meeko 的独立 Python 环境。
...
DockStart 的长期方向（不代表 V0.9.7 Basic Stable 当前已包含这些能力）：

- 分发简单：优先减少 Basic Mode 的安装与路径配置步骤。
- 工具链分层：V0.9.7 随附 Vina 与后端 Python；RDKit/Meeko 仍由用户为 Assisted Mode 单独配置。
- 开箱即用边界：对已经有受体/配体 PDBQT 的用户，Basic Mode 可直接使用；原始结构自动准备不属于本版本的零配置能力。
...
- V0.9.7 Basic Stable 随附 AutoDock Vina；源码运行或随附 Vina 检测失败时，也可以在设置页配置外部 Vina；
...
## DockStart Full 工具链方向（未来规划）

后续 Full 版本计划采用分层工具链；这不是 V0.9.7 Basic Stable 的当前发布能力：

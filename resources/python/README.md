...
DockStart Basic 候选 profile 的安装包只分发运行 Python 后端所需的精简 CPython：
...
因此 Basic 候选 profile 中，如果用户需要从 PDB/SDF 自动准备 PDBQT，仍需在
设置页配置包含 RDKit 与 Meeko 的独立 Python 环境。缺少这些包不会阻止已有
PDBQT 的 Basic Mode。

Assisted 候选 profile 使用单独的 `.release/assisted/` 白名单 stage，从固定离线

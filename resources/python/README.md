...
DockStart v0.12.0 Basic Stable 的安装包只分发运行 Python 后端所需的精简 CPython：
...
因此 v0.12.0 Basic Stable 中，如果用户需要从 PDB/SDF 自动准备 PDBQT，仍需在
设置页配置包含 RDKit 与 Meeko 的独立 Python 环境。缺少这些包不会阻止已有
PDBQT 的 Basic Mode。

v0.12.0 Assisted Stable 使用单独的 `.release/assisted/` 白名单 stage，从固定离线
